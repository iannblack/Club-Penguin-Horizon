...
export default class PenguinLogin extends BaseScene {
    ...
    _create() {
        ...
        forgotButtonSimpleButton.callback = () => this.onForgetPasswordClick();
        ...
        loginButtonButton.activeFrame = true;
    ...
    create(data) {
        ...
        this.passwordInput = new TextInput(this, 973, 250, 'password', style, () => this.onLoginSubmit(), 128, false, undefined, /(?!.*)/gi)
    ...
    onLoginSubmit() { 
        ...
        if (token && !this.passwordEdited) {
            onConnect = () => this.network.send('token_login', { username: username, token: token, secret: secret })
        } else {
            onConnect = () => this.network.send('login', { username: username, password: password, secret: secret })
        }
