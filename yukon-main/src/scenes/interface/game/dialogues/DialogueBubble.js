...
export default class DialogueBubble extends Phaser.GameObjects.Container {
    ...
    constructor(scene, x, y) {
        super(scene, x ?? 439.79254072753525, y ?? 766.405908436523);
        ...
        // bubble
        const bubble = scene.add.image(452.20745927246475, 9.594091563477036, "dialogue-common", "textBubble");
        bubble.scaleX = 0.8;
        bubble.scaleY = 0.8;
        this.add(bubble);

        // text
        const text = scene.add.text(-38, -124, "", {});
        text.text = "Waddle On!";
        text.setStyle({ "color": "#333", "fixedWidth":1050,"fixedHeight":250,"fontFamily": "Burbank Small", "fontSize": "40px", "fontStyle": "bold" });
        text.setPadding({"left":10});
        text.setWordWrapWidth(1050);
        this.add(text);

        // options
        const options = scene.add.container(489, -30);
        ...
        // option1
        const option1 = scene.add.nineslice(0, -59, "dialogue-common", "button", 1000, 74, 20, 20, 20, 20);
        options.add(option1);

        // option2
        const option2 = scene.add.nineslice(0, 23, "dialogue-common", "button", 1000, 74, 20, 20, 20, 20);
        options.add(option2);

        // option3
        const option3 = scene.add.nineslice(0, 104, "dialogue-common", "button", 1000, 74, 20, 20, 20, 20);
        options.add(option3);

        // option1Text
        const option1Text = scene.add.text(-488, -84, "", {});
        option1Text.text = "Can i just go back to the cave mine please";
        option1Text.setStyle({ "color": "#333", "fixedWidth":1000,"fixedHeight":50,"fontFamily": "Burbank Small", "fontSize": "43px", "fontStyle": "bold" });
        option1Text.setPadding({"left":10});
        option1Text.setWordWrapWidth(1050);
        options.add(option1Text);

        // option2Text
        const option2Text = scene.add.text(-488, -1, "", {});
        option2Text.text = "Cool story bro but did i ask";
        option2Text.setStyle({ "color": "#333", "fixedWidth":1000,"fixedHeight":50,"fontFamily": "Burbank Small", "fontSize": "43px", "fontStyle": "bold" });
        option2Text.setPadding({"left":10});
        option2Text.setWordWrapWidth(1050);
        options.add(option2Text);

        // option3Text
        const option3Text = scene.add.text(-488, 79, "", {});
        option3Text.text = ":headmod:";
        option3Text.setStyle({ "color": "#333", "fixedWidth":1000,"fixedHeight":50,"fontFamily": "Burbank Small", "fontSize": "43px", "fontStyle": "bold" });
        option3Text.setPadding({"left":10});
        option3Text.setWordWrapWidth(1050);
        options.add(option3Text);

        // bubbleButton
        const bubbleButton = scene.add.rectangle(484, -3, 1100, 280);
