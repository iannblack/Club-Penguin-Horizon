...
export default class MailbookUserItem extends BaseContainer {
    ...
    constructor(scene, x, y) {
        ...
        // item
        const item = scene.add.image(0, 0, "mail-book-buddy", "list/item");
        this.add(item);

        // icon
        const icon = scene.add.image(-139, 0, "mail-book-buddy", "list/icon_offline");
        this.add(icon);
    ...
    get mailbook() {
        return this.interface.loadedWidgets.MailBook
    }
    ...
    onClick() {
        if (this.id) {
            this.mailbook.showMainView(this.id, this.username.text)
        }
    }
