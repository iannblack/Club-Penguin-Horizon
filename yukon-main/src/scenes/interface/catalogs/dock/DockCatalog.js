...
export const preload = {
    key: 'dockcatalog-pack',
    url: 'assets/media/interface/catalogs/dock/dockcatalog-pack.json',
    loadString: ['loading', 'dockcatalog']
}

/* START OF COMPILED CODE */

export default class DockCatalog extends BookContainer {
    ...
    constructor(scene, x, y) {
        ...
        // bg
        const bg = scene.add.image(0, 0, "dockcatalog", "dock");
        bg.setOrigin(0, 0);
        page1.add(bg);

        // closebtn
        const closebtn = scene.add.image(1041, 93, "dockcatalog", "closebtn");
        page1.add(closebtn);

        // tag2
        const tag2 = new Tag2(scene, 573, 320);
        tag2.angle = 22;
        page1.add(tag2);

        // tag
        const tag = new Tag2(scene, 708, 413);
        tag.angle = 26;
        page1.add(tag);
