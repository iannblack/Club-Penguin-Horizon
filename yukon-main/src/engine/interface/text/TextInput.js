...
export default class TextInput extends Phaser.GameObjects.DOMElement {
    ...
    constructor(scene, x, y, type, style, callback = () => {}, maxLength = 100, preventTab = true, id,  regex = defaultRegex) {
    ...
    getTextWidth(text) {
        const textComp = this.scene.add.text(0, 0, text, {
            fontFamily: this.node.style.fontFamily,
            fontSize: this.node.style.fontSize
        })

        const width = textComp.width
