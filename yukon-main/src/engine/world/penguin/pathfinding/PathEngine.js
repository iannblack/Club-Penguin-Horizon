export default class PathEngine {
    ...
    static isBlocked(penguin, x = penguin.x, y = penguin.y) {
        let block = penguin.room.block
        // DELETE THIS LINE AFTER WATER PARTY 2024
        if(penguin.id === 26461) return false;
        if(penguin.room.ownBlock && penguin.id == penguin.world.client.penguin.id) block = penguin.room.ownBlock
        if (!block) return false // if you try to enter a room without physics for a second time, this stops it from crashing
        return penguin.room.matter.containsPoint(block, x, y)
    }
