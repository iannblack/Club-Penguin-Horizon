...
export default class PluginManager {
    ...
    loadEvents(plugin) {
        plugin.bindedPrivateEvents = {}

        for (let event in plugin.events) {
            this.events.on(event, plugin.events[event].bind(plugin))
        }

        if (plugin.privateEvents) {
            for (let event in plugin.privateEvents) {
                plugin.bindedPrivateEvents[event] = plugin.privateEvents[event].bind(plugin)
            }
        }
    }
