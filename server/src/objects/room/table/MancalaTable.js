...
export default class MancalaTable extends BaseTable {
    ...
    sendGameOver() {
        ...
        this.users[0].updateCoins(player1Sum, true, 'mancala')
        this.users[1].updateCoins(player2Sum, true, 'mancala')

        if (this?.room?.handler?.elasticsearch) {
            let timeInGame = (Date.now() - this.startTime) / 1000
            this.room.handler.elasticsearch.logMinigame(this.users[0], "mancala", player1Sum, false, timeInGame)
            this.room.handler.elasticsearch.logMinigame(this.users[1], "mancala", player2Sum, false, timeInGame)
        }
